...
const ShowList = ({ shows }) => {
  return (
    <div style={{backgroundColor:'black'}}>
      <header style={{ backgroundColor: '#e50914', padding: '10px', textAlign: 'center' }}>
        <h1 style={{ color: 'white' }}>Show List</h1>
      </header>
      <ul style={{ listStyleType: 'none', padding: 0, display: 'flex', flexWrap: 'wrap', marginBottom:'0px' }}>
        {shows.map((show) => (
          <li key={show.show.id} style={{ margin: '10px', flex: '1 0 200px', textAlign: 'center' }}>
            <Link
            ...
            >
              <img
                src={show.show.image ? show.show.image.medium : getRandomImage()}
                alt={show.show.name}
